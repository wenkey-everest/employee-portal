...
const EmployeeTable = () => {
    ...
    const deleteAllEmployees = (id) => {
        api.deleteAllEmployees().then(response => {
    ...
    const searchEmployee = () =>{
            api.searchEmployee(name).then(response=>{
                setEmployees(response.data.data);
    ...
    return (
        ...
        <div className="input-group-append">
          ...
          >
            ...
            <hr/>
            <Button className= "btn btn-warning col-md mt-3"  onClick={()=>{nav("/saveEmployee")}}>Add Employee</Button>
            <Button className= "btn btn-warning ms-4 mt-3 col-md"  onClick={()=>deleteAllEmployees()}>Delete All Employees</Button>
                    ...
                    {employees.map(emp => {
                        return (
                            <tr key={emp.empId}>
                                ...
                                <td>{emp.bio}</td>
                                <td><button className={"btn btn-warning"} onClick={()=>{nav("/updateEmployee/"+emp.empId)}}>Edit</button></td>
